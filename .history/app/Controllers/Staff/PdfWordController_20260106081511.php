...
class PdfWordController extends ProtectedController
{
    ...
    public function generateWordFromLetter($id)
    {
        ...
        // Cek tipe surat
        if ($letter['tipe_surat'] !== 'Keterangan Usaha' && $letter['tipe_surat'] !== 'Keterangan Tidak Mampu') {
            return redirect()->to('/staff/surat/' . $id)->with('error', 'Export hanya tersedia untuk Surat Keterangan Usaha dan Keterangan Tidak Mampu.');
        }
    ...
    private function generateKeteranganTidakMampu($section, $senderProfile, $staffProfile, $tempatTanggalLahirSender, $tanggalSekarang, $bulan)
    {
        ...
        $cell0->addText('', ['size' => 11, 'name' => 'Arial']);
        $cell1 = $senderTable->addCell(2500, $cellStyleNoBorder);
        $cell1->addText('NIK', ['size' => 11, 'name' => 'Arial']);
        $cell2 = $senderTable->addCell(300, $cellStyleNoBorder);
        $cell2->addText(':', ['size' => 11, 'name' => 'Arial']);
        $cell3 = $senderTable->addCell(6000, $cellStyleNoBorder);
        $cell3->addText($this->getProfileValue($senderProfile, 'nik'), ['size' => 11, 'name' => 'Arial']);

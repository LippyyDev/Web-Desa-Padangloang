...
<div class="page-header">
    <div>
        ...
        <div class="text-muted small">Kelola data perangkat desa.</div>
    </div>
    <div class="page-header-actions">
        <div class="page-header-icon">
            <i class="bi bi-people"></i>
        </div>
        <a href="<?= base_url('/staff/perangkat-desa/tambah') ?>" class="page-header-icon page-header-icon-add" title="Tambah Perangkat Desa">
            <i class="bi bi-plus-circle"></i>
        </a>
    </div>
</div>

<div class="d-flex flex-column flex-md-row justify-content-start align-items-start align-items-md-center gap-3 mb-4">
    <div class="col-12 col-md-5">
        <div class="input-group">
            <span class="input-group-text"><i class="bi bi-search"></i></span>
            <input type="text" id="searchInput" class="form-control" placeholder="Cari perangkat desa...">
                ...
                if (response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(function(item) {
                        html += '<div class="col-md-6 col-lg-4 col-xl-3">' +
                            ...
                            (item.kontak ? '<p class="text-muted small mb-3"><i class="bi bi-telephone"></i> ' + item.kontak + '</p>' : '') +
                            '<div class="d-flex gap-2 justify-content-center">' +
                            '<a href="<?= base_url('/staff/perangkat-desa/') ?>' + item.id + '/edit" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i> Edit</a> ' +
                            '<a href="<?= base_url('/staff/perangkat-desa/') ?>' + item.id + '/hapus" class="btn btn-sm btn-outline-danger" onclick="return confirm(\'Hapus data ini?\')"><i class="bi bi-trash"></i> Hapus</a>' +
                            '</div>' +
                            '</div>' +
                            '</div>' +
                            '</div>';
